...
class ContactAjaxHandler extends SanitizeAndValidate {
    ...
    public function handle_form_submission() {
        $this->_data = $this->main($this->_data, $_POST, 'contact_form_nonce');
        $this->_response();
    }

    public function _response() {
        $site_name = get_bloginfo('name');
        $subject = sprintf('[%s] New Contact Submission', $site_name);
        ...
        if ($sent) {
            wp_send_json_success(['message' => 'Thank you — your message submission was sent.']);
        } else {
    ...
    public function buildEmailTemplate($site_name, $data){
        ...
        return '
            ...
            <div class="wrap">
                ...
                <table>
                    <tr><td class="label">Name</td><td class="value">' . ($fields['name'] ?: '—') . '</td></tr>
                    <tr><td class="label">Email</td><td class="value">' . ($fields['email'] ?: '—') . '</td></tr>
                    <tr><td class="label">Message</td><td class="value">' . ($fields['message'] ?: '—') . '</td></tr>
                </table>
                </div>
                <div class="footer">
